...
public class UserProfileUtil {
	...
	public UserProfileResponseObject getUserProfileUtility(JSONObject userProfileResponse) throws Exception{
		...
		try{
			...
			if(userProfileResponse.has("responseCode")){
				...
				if (userProfileResponse.getString("responseCode").equals("401") && userProfileResponseObject.getMpxRetry()==1) {
					...
					DAAGlobal.LOGGER.error(DAAConstants.MPX_403_CODE+"||"+DAAConstants.MPX_403_MESSAGE);
	...
	public static UserProfileWithDeviceRequestObject populateUserProfileFromReq(NGCAReqObject ngcaReqObject) {
		
		String deviceId = ngcaReqObject.getDeviceIdOfReqDevice();
		String vsid = ngcaReqObject.getVSID();
		String correlationID = ngcaReqObject.getCorrelationId();

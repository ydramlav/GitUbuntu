...
public class RetrievePhysicalDeviceFromSolrMaster {


	public PhysicalDeviceInfoResponseObject getPhysicalDeviceFromSolrMaster(PhysicalDeviceInfoRequestObject requestObject) throws Exception{
		...
		try {
			...
			uri = new URIBuilder().setScheme(DAAGlobal.solrProtocol).setHost(DAAGlobal.solrPhysicalDeviceMasterHost).setPath(DAAGlobal.solrURIDeviceInfo).addParameters(urlqueryParams).build();

			//uri = URIUtils.createURI(DAAGlobal.solrProtocol,DAAGlobal.solrPhysicalDeviceMasterHost, -1,DAAGlobal.solrURIDeviceInfo, URLEncodedUtils.format(urlqueryParams,"UTF-8"), null);

			DAAGlobal.LOGGER.info("Request to Solr Master PhysicalDevice End Point - URI :: "+uri );
			...
			DAAGlobal.LOGGER.info("ResponseCode from Solr Master ::"+responseCode+" - Response Json from Solr Master :: "+ fin);
			if (responseCode == 200  ) {
				...
				physicalDeviceResponseObject.setErrorCode(String.valueOf(responseCode));
				physicalDeviceResponseObject.setErrorMsg(DAAConstants.DAA_1027_MESSAGE);
			}
		} catch (IOException e) {
			stringWriter = new StringWriter();
			e.printStackTrace(new PrintWriter(stringWriter));
			DAAGlobal.LOGGER.error("IOException while retrieving physicalDevices from Solr Master :: " + stringWriter.toString() );
			new VOSPGenericException(DAAConstants.DAA_1006_CODE, DAAConstants.DAA_1006_MESSAGE+"||"+e.getMessage());
		} catch (JSONException e) {
			stringWriter = new StringWriter();
			e.printStackTrace(new PrintWriter(stringWriter));
			DAAGlobal.LOGGER.error("JSONException while retrieving physicalDevices from Solr Master :: " + stringWriter.toString() );
			new VOSPGenericException(DAAConstants.DAA_1006_CODE, DAAConstants.DAA_1006_MESSAGE+"||"+e.getMessage());
		} catch (URISyntaxException e) {
			stringWriter = new StringWriter();
			e.printStackTrace(new PrintWriter(stringWriter));
			DAAGlobal.LOGGER.error("URISyntaxException while retrieving PhysicalDevice from Solr Master :: " +stringWriter.toString() );
			new VOSPGenericException(DAAConstants.DAA_1006_CODE, DAAConstants.DAA_1006_MESSAGE+"||"+e.getMessage());
		} catch (Exception e) {
			stringWriter = new StringWriter();
			e.printStackTrace(new PrintWriter(stringWriter));
			DAAGlobal.LOGGER.error("Exception while retrieving physicalDevices from Solr Master :: " +stringWriter.toString() );
			new VOSPGenericException(DAAConstants.DAA_1006_CODE, DAAConstants.DAA_1006_MESSAGE+"||"+e.getMessage());
		}
		finally{
			try {
